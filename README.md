...
- 다양한 실제 서비스 흐름을 검증하는 방대한 API 통합 테스트 구성(`app-api` 모듈)
- 테스트 전략, `Fixture` 기반 테스트, 부하 테스트 등 입증 가능한 테스트 환경
- AI를 적극 활용한 개발 및 생산성 극대화(테스트 데이터 생성, 프론트엔드 구현 등)
...
## AI를 활용한 개발 전략
Threadly는 개발 효율과 생산성을 높이기 위해 **AI를 개발 조수로 적극 활용했습니다.**
...
단순히 반복되는 구조의 테스트 작성 부담을 AI가 크게 줄여 주었습니다.

<br>

### 프론트엔드 개발에서의 AI 활용

**Threadly는 백엔드 프로젝트이기에** 프론트엔드 구현에 대한 고민이 있었습니다.

그러나 AI를 이용해 **`React` 경험이 거의 없는 상황에서 웹 프론트엔드를 구현했습니다.**

AI가 기본적인 골격을 만들면, 

개발자가 직접 백엔드 스펙과 상태 흐름에 맞춰 오류를 분석하고 필요한 부분을 직접 수정하는 방식으로 실서비스까지 구현했습니다.
